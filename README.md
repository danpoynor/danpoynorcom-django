...
- <http://localhost:8000/wget_sitemap/>: Generates an alternate sitemap for `wget` to use when generating static files. NOTE: Due to a bug in this script creating links to 404 pages I'm still relying on the links in `sitemap.xml` to generate the static files.
